...
def fft_analysis(in_file, peaks, tr):
    import os
    from functions.functions import nextpow2
    import numpy as np
    ...
    nfft = 2^nextpow2(data.shape[3])
    Fs=1/tr
    f=Fs/2*np.linspace(0,1,int(nfft/2+1))

    data_fft = np.absolute(np.fft.rfft(data, nfft)/data.shape[3])
    map_fft = np.sum(data_fft[:,:,:,peaks], axis=3)

    _, base, _ = split_filename(in_file)
    img_fname = base + '_fft.nii'
    nb.save(nb.Nifti1Image(map_fft, img.affine, img.header), img_fname)

    return os.path.abspath(img_fname)
